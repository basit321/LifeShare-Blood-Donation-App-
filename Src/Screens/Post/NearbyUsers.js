...
const NerbyUsers = ({ navigation, route }) => {
  const { lat, long, bloodgroup, id } = route.params;
  ...
  const notification = (token, item) => {
    if (token == null && item == null) {
      Alert.alert("Something went wrong");
    }
    const title = "Blood Required";
  ...
  const sendNotifications = async () => {
    let filterobject = datas.filter(
      (item) =>
        item.dist < 10000 && item.bloodgroup == bloodgroup && item._id != id
    );
  ...
  return (
    <View style={styles.container}>
      ...
      <Headersub
        tittle="Nreby Users"
        onBackbuttonpress={() => Navigation.navigate("Posts")}
      />
      {datas.length >= 1 ? (
        <ScrollView style={styles.subview}>
          <Text style={styles.text2}>Near by Users</Text>
          ...
          {datas.filter(
            (item) =>
              item.dist < km && item.bloodgroup == bloodgroup && item._id != id
          ).length >= 1 ? (
            datas
              .filter(
                (item) =>
      ...
      <Button
        tittle="Check Neraby Blood Banks"
